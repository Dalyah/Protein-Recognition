...
def calculate_transformation_parameters(point1, point2, point3, bin_size):
    ...
    # rot_y_deg = rot_y_rad * 180.0 / math.pi
    ...
    if rot_y_rad != 0:
        ...
        # new_x2 = int((temp_x * 10000 + .5) / 10000.0)
        # new_z2 = int((temp_z * 10000 + .5) / 10000.0)

        new_x3 = new_x3 * cos_theta + new_z3 * sin_theta
        new_z3 = -1 * new_x3 * sin_theta + new_z3 * cos_theta
        # new_x3 = int((temp_x * 10000 + .5) / 10000.0)
        # new_z3 = int((temp_z * 10000 + .5) / 10000.0)
    ...
    # rot_z_deg = rot_z_rad * 180.0 / math.pi
    if rot_z_rad != 0:
        ...
        # new_x2 = int((tmpx * 10000 + .5) / 10000.0)
        # new_y2 = int((tmpz * 10000 + .5) / 10000.0)  # this should be zero

        new_x3 = new_x3 * cos_theta + -1 * new_y3 * sin_theta
        new_y3 = new_x3 * sin_theta + new_y3 * cos_theta
        # new_x3 = int((tmpx * 10000 + .5) / 10000.0)
        # new_y3 = int((tmpz * 10000 + .5) / 10000.0)
    ...
    # # rot_x_deg = rot_x_rad * 180.0 / math.pi
    # if rot_x_rad != 0:
    #     sin_theta = math.sin(-1 * rot_x_rad)
    #     cos_theta = math.cos(-1 * rot_x_rad)
    #
    #     new_y3 = new_y3 * cos_theta + -1 * new_z3 * sin_theta
    #     new_z3 = new_y3 * sin_theta + new_z3 * cos_theta
    #     # new_y3 = int((tmpx * 10000 + .5) / 10000.0)
    #     # new_z3 = int((tmpz * 10000 + .5) / 10000.0)

    scale = new_x2
    # scale2 = math.sqrt(new_x3 ** 2 + new_y3 ** 2)
    # if new_x2 < 0 and new_x2 > -1:
    #     new_x2 = 0
    # else:
    #     new_x2 = round(new_x2 / bin_size)
    #
    # if new_y2 < 0 and new_y2 > -1:
    #     new_y2 = 0
    # else:
    #     new_y2 = round(new_y2 / bin_size)
    #
    # if new_z2 < 0 and new_z2 > -1:
    #     new_z2 = 0
    # else:
    #     new_z2 = round(new_z2 / bin_size)
...
def transform_point3d(trans_info, p, bin_size):
    ...
    # p.x = int((tmpx * 10000 + .5) / 10000.0)
    # p.z = int((tmpz * 10000 + .5) / 10000.0)
    ...
    #     p.x = 0
    # else:
    #     p.x = round(p.x / bin_size)
    #
    # if p.y < 0 and p.y > -1:
    #     p.y = 0
    # else:
    #     p.x = round(p.y / bin_size)
    #
    # if p.z < 0 and p.z > -1:
    #     p.z = 0
    # else:
    #     p.z = round(p.z / bin_size)
    return p
            ...
            for z in test_htable[x][y]:
                ...
                for model_id in full_htable[x][y][z]:
                    for b1, b2, b3 in full_htable[x][y][z][model_id]:
                        ...
                        # TODO loop through all of them not onlyfirst one
